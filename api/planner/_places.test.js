// /api/planner/places 핸들러 테스트 (2026-09-05, 구글 분기 + fail-closed. 호출 한도는 쿠마님 결정으로 없음).
...
function fakeSupabase({ cached = null, cacheError = null, geoSlot = 0 } = {}) {
  const calls = [];
  const supabase = {
    calls,
    rpc: async (name, args) => {
...
describe('구글 분기', () => {
  it('캐시 미스: 한도 RPC 없이 바로 한 번 호출(필드마스크 고정), 정규화 결과 + 캐시 저장', async () => {
    ...
    expect(rpcNames(sb)).toEqual([]); // 한도 예약도 OSM 슬롯도 부르지 않는다
  ...
  it('캐시 히트: 호출 없이 cached:true', async () => {
  ...
  it('서버 키 없음: OSM 강등 없이 503', async () => {
