...
// 보이는 카드 수에 맞춘 데스크톱(lg↑) 열 수 — 마지막 줄에 카드 하나만 덩그러니 남는 3+1 배치를 막는다(2026-09-07 쿠마님).
// 4장(일반 방문자) = 4열 한 줄, 5장(승무원) = 5열 한 줄, 6장 = 3×2, 7·8장 = 4열(4+3 / 4+4 — 3열이면 7장이 3+3+1 로 되돌아감).
// 카테고리를 더 늘리면 이 표도 같이 늘릴 것. Tailwind JIT 는 문자열 그대로 있어야 하므로 표로 둔다.
const DESKTOP_COLS = {
    1: 'lg:grid-cols-1', 2: 'lg:grid-cols-2', 3: 'lg:grid-cols-3',
    4: 'lg:grid-cols-4', 5: 'lg:grid-cols-5', 6: 'lg:grid-cols-3',
    7: 'lg:grid-cols-4', 8: 'lg:grid-cols-4',
};
...
const CategoryBoard = ({ activeCategory, onCategoryChange }) => {
    ...
    const desktopCols = DESKTOP_COLS[list.length] || 'lg:grid-cols-3';
    // 모바일·태블릿은 2열 고정. 홀수면 마지막 카드가 한 줄을 다 쓰게(가로 2:1) 해서 반쪽 줄을 없앤다.
    ...
    return (
        <section className="bg-white py-14 sm:py-20">
            <div className="max-w-content mx-auto px-4 sm:px-6">
                ...
                <div className={`grid grid-cols-2 ${desktopCols} gap-4 sm:gap-6`}>
                    {list.map((cat, i) => {
                        ...
                        <Link
                            ...
                            className={`group block card-air overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink${spans ? ' col-span-2 lg:col-span-1' : ''}`}
                        >
                            <div className={`${spans ? 'aspect-[2/1] lg:aspect-[4/3]' : 'aspect-[4/3]'} overflow-hidden bg-surface-strong`}>
                                <img
                                    src={cat.image}
                                    alt={cat.name}
                                    loading="lazy"
