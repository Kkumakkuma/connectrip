...
--
--   ⚠ 알려진 짧은 창: 배포 전환 중에 "새 인스턴스가 보낸 코드"를 "아직 살아 있는 옛 인스턴스"가
--     검증하면 p_code_hash 를 안 보내므로 불일치가 난다(짧게는 수 초, Vercel 구 인스턴스가 남으면
--     몇 분). 사용자는 재발송하면 된다. 운영 적용(2026-09-05)은 OTP 테이블이 비어 있고 트래픽이
--     거의 없는 시간에 했다. 적용은 MCP apply_migration(요청 단위 트랜잭션)으로 Vault·RPC 와
--     컬럼·함수 교체를 두 번에 나눠 실행했다(codex 지적 반영).
